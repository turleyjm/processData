...
class GUI:
    ...
    def run(self):
        ...
        self._create_file_selection(
            root, row, FileType.STACK, "Select Stack file", default=self.stack_path
        )

        row += 1
        self._create_heading(root, row, "Focus parameters")
        row += 1
        self._focus_range_en = self._create_input(
            root, row, "Focus range", default=self.focus_range
        )
        ...
        self._create_file_selection(
            ...
            row,
            FileType.ECAD_MODEL,
            "Select Ecad model file",
            default=self.ecad_model_path,
        )
        row += 1
        self._create_file_selection(
            root,
        ...
        row += 1
        self._save_focus_cb = self._create_checkbutton(
            root, row, "Save focus images", default=self.save_focus
        )
        row += 1
        self._save_norm_cb = self._create_checkbutton(
            root, row, "Save normalised images", default=self.save_norm
        )
        row += 1
        self._save_prob_cb = self._create_checkbutton(
            root, row, "Save probabiliy images", default=self.save_prob
        )
